...
let AdminGuardService = class AdminGuardService {
    ...
    canActivate(next, state) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            try {
                return !!(yield this.databaseService.isAuthenticated());
            }
            catch (_a) {
...
/* harmony import */ var ngx_doc_viewer__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! ngx-doc-viewer */ "./node_modules/ngx-doc-viewer/fesm2015/ngx-doc-viewer.js");
/* harmony import */ var angular_star_rating__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(/*! angular-star-rating */ "./node_modules/angular-star-rating/esm2015/angular-star-rating.js");
...
let CandidateDetailsComponent = class CandidateDetailsComponent {
    constructor(route, databaseService, spinner, toastr, emailService) {
        this.route = route;
        this.databaseService = databaseService;
    ...
    addInterview() {
        ...
        if (this.interviewAction == 'Add') {
            ...
            this.phoneInterViews[this.currentInterviewEditIndex] = Object.assign({}, this.interview);
        }
        message += `
...
let CandidatesComponent = class CandidatesComponent {
    constructor(DatabaseService, router, spinner, toastr) {
        this.DatabaseService = DatabaseService;
        this.router = router;
        this.spinner = spinner;
        this.toastr = toastr;
        ...
        this.DatabaseService.getInitialCandidates(data => {
            ...
            if (this.DatabaseService.user.level == 1)
                this.candidates = this.candidates.filter(element => element.asignor == this.DatabaseService.user.id);
    ...
    filter(type) {
        ...
        if (type !== 'All')
            this.candidates = this.candidates.filter(el => el.stage == type);
        this.rerender();
    }
    rerender() {
...
let AddJobopeningComponent = class AddJobopeningComponent {
    constructor(DatabaseService, route, spinner, router, toastr) {
        this.DatabaseService = DatabaseService;
        this.route = route;
        this.spinner = spinner;
        this.router = router;
...
let ClientNavbarComponent = class ClientNavbarComponent {
    ...
    toggleSidebar() {
        console.log('toggle');
        const sidebar = this.injector.get(_sidebar_sidebar_component__WEBPACK_IMPORTED_MODULE_2__["SidebarClientComponent"]);
...
let HomeComponent = class HomeComponent {
    constructor(DatabaseService, spinner, toastr, emailService, formBuilder) {
        this.DatabaseService = DatabaseService;
        this.spinner = spinner;
        this.toastr = toastr;
        this.emailService = emailService;
    ...
    applyNow() {
        ...
        if (!this.registerForm.invalid) {
            ...
            this.fields.forEach(el => {
                if (el.type == 'file') {
                    const fileEl = document.getElementById(this.removeSpaces(el.text));
    ...
    doApplyNow(jobOpening) {
        ...
        this.fields.forEach(element => {
            validation[this.removeSpaces(element.text)] = ['', element.isRequired ? _angular_forms__WEBPACK_IMPORTED_MODULE_8__["Validators"].required : _angular_forms__WEBPACK_IMPORTED_MODULE_8__["Validators"].nullValidator];
        });
...
let MissingRequirmentsComponent = class MissingRequirmentsComponent {
    constructor(databaseService, route, spinner, emailService, toastr) {
        this.databaseService = databaseService;
        this.route = route;
        this.spinner = spinner;
        this.emailService = emailService;
...
let SidebarClientComponent = class SidebarClientComponent {
    constructor() {
...
let DashBoardComponent = class DashBoardComponent {
    constructor(route, databaseService, spinner) {
        this.route = route;
        this.databaseService = databaseService;
        this.spinner = spinner;
...
let DepartmentsComponent = class DepartmentsComponent {
    constructor(DatabaseService, spinner, toastr) {
        this.DatabaseService = DatabaseService;
        this.spinner = spinner;
        this.toastr = toastr;
        this.dtOptions = {};
    ...
    addDepartment() {
        ...
        this.DatabaseService.addDepartment(this.departmentTitle, results => {
            console.log(results);
            this.departments.push({ title: this.departmentTitle, id: results.insertId, no_of_employee: 0 });
...
let EmployeesComponent = class EmployeesComponent {
    ...
    filter(type) {
        ...
        if (type !== 'All')
            this.employees = this.employees.filter(el => el.status == type);
        this.rerender();
    }
    rerender() {
        console.log('rerender');
...
let JobopeningComponent = class JobopeningComponent {
    constructor(DatabaseService, router, spinner, toastr) {
        this.DatabaseService = DatabaseService;
        this.router = router;
        this.spinner = spinner;
...
let LogInComponent = class LogInComponent {
    constructor(databaseService, spinner, router) {
        this.databaseService = databaseService;
        this.spinner = spinner;
        this.router = router;
        this.user = {
...
let NavBarComponent = class NavBarComponent {
    constructor(injector, router, databaseService) {
        this.injector = injector;
        this.router = router;
        this.databaseService = databaseService;
        console.log(this.databaseService.user);
    ...
    toggleSidebar() {
        console.log('toggle');
        const sidebar = this.injector.get(_sidebar_sidebar_component__WEBPACK_IMPORTED_MODULE_2__["SidebarComponent"]);
        sidebar.toggleSidebar();
    }
    logOut() {
...
let QuestionsetsComponent = class QuestionsetsComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let ReportsComponent = class ReportsComponent {
    constructor(databaseService, spinner, csvService) {
        this.databaseService = databaseService;
        this.spinner = spinner;
...
let CsvGeneratorService = class CsvGeneratorService {
    constructor() { }
...
let DatabaseService = class DatabaseService {
    constructor(spinner, toastr, http) {
        this.spinner = spinner;
        this.toastr = toastr;
        this.http = http;
    ...
    addJobOpening(data, callback) {
        this.socket.emit('add-job-opening', data, callback);
    }
    updateJobOpening(data, callback) {
    ...
    getCandidateDetails(id, callback) {
        this.socket.emit('get-candidate', id, callback);
    }
    getCandidateDetailsByUid(uid, callback) {
        this.socket.emit('get-candidate-by-uid', uid, callback);
    }
    getJobStages(id, callback) {
