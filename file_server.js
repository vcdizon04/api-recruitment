...
app.post('/api/upload', multipartMiddleware, async (req, res) => {
    ...
    db.query(`INSERT INTO candidates (id, created_on, job_opening_id, job_title, stage, rating, details, interviews, to_dos, status) VALUES (NULL, ${dateNow}, ${data.job_opening_id}, '${data.job_title}', '${data.stage}', 0.00, '${JSON.stringify(data.details)}', '[]', '[]', '')`, (err, results) => {
        if(err) {
            console.log(err)
        }
        res.json(data);
